...
def invoice(request):
    odc=OtwDc.objects.filter(gcn_no='084/2023-24')
    odc1=get_object_or_404(OtwDc,po_sl_no='2',gcn_no='084/2023-24')
    ...
    total_qty = OtwDc.objects.filter(gcn_no='084/2023-24').aggregate(total_qty=Sum('qty_delivered'))['total_qty']
    total_taxable_value =OtwDc.objects.filter(gcn_no='084/2023-24').aggregate(total_taxable_value=Sum('taxable_amt'))['total_taxable_value']
    total_cgst = OtwDc.objects.filter(gcn_no='084/2023-24').aggregate(total_cgst=Sum('cgst_price'))['total_cgst']
    total_sgst = OtwDc.objects.filter(gcn_no='084/2023-24').aggregate(total_sgst=Sum('sgst_price'))['total_sgst']
    total_igst = OtwDc.objects.filter(gcn_no='084/2023-24').aggregate(total_igst=Sum('igst_price'))['total_igst']
