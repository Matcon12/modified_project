...
if inw(mydb, mycursor, grn):
    ...
    mycursor.execute("SELECT grn_no, grn_date, po_no, po_date, receiver_id, po_sl_no, part_id, qty_delivered, uom, part_name FROM inw_dc WHERE grn_no=%s AND po_sl_no IN ({})".format(','.join(map(str, po_sl_numbers))), (grn,))
    ...
    for idx, row in enumerate(data_inw):
        ...
        if state_code == 29:
            cgst_price = '{:.2f}'.format( 0.09 * 0)
            sgst_price = '{:.2f}'.format( 0.09 * 0)
            igst_price = 0   
        else:
            cgst_price = 0  
            sgst_price = 0  
            igst_price = '{:.2f}'.format( 0.18 * 0)
        
        insert_row = (code, gcn_num,date)+ row + (0,0, 0, 0, 0, 1,y) 
        insert_data.append(insert_row)
        
    insert_query = """
                INSERT INTO otw_dc 
                (mat_code, gcn_no, gcn_date, grn_no, grn_date, po_no, po_date, receiver_id, po_sl_no, part_id, qty_delivered, uom,part_name,unit_price, taxable_amt, cgst_price, sgst_price, igst_price,rejected_item,consignee_id) 
